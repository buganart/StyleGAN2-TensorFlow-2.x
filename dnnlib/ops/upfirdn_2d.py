...
def _upfirdn_2d_ref(x, k, upx, upy, downx, downy, padx0, padx1, pady0, pady1, gpu=True):
    ...
    if gpu:
        x = tf.nn.conv2d(
            x, w, strides=[1, 1, 1, 1], padding="VALID", data_format="NCHW"
        )
    else:
...
def _upfirdn_2d_cuda(
    x, k, upx, upy, downx, downy, padx0, padx1, pady0, pady1, gpu=True
):
...
def upsample_conv_2d(
    x, w, k=None, factor=2, gain=1, data_format="NCHW", impl="cuda", gpu=True
):
...
def _simple_upfirdn_2d(
    x, k, up=1, down=1, pad0=0, pad1=0, data_format="NCHW", impl="cuda", gpu=True
):
