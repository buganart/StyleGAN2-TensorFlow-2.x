...
from utils.weights_map import (
    available_weights,
    synthesis_weights,
    mapping_weights,
    weights_stylegan2_dir,
)
...
class MappingNetwork(tf.keras.layers.Layer):
    ...
    def call(self, z):
        ...
        # Normalize inputs
        scale = tf.math.rsqrt(
            tf.reduce_mean(tf.square(z), axis=1, keepdims=True) + 1e-8
        )
        ...
        for i in range(self.mapping_layers):
            ...
            x = tf.math.multiply(tf.nn.leaky_relu(x, 0.2), tf.math.sqrt(2.0))
...
class SynthesisNetwork(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        # early layer 4x4
        self.layer_4_4 = SynthesisMainLayer(
            fmaps=nf(1), impl=self.impl, gpu=self.gpu, name="4x4"
        )
        ...
        for res in range(3, self.resolution_log2 + 1):
            ...
            setattr(
                self,
                "layer_{}_{}_up".format(res_str, res_str),
                SynthesisMainLayer(
                    fmaps=nf(res - 1),
                    impl=self.impl,
                    gpu=self.gpu,
            ...
            setattr(
                self,
                "layer_{}_{}".format(res_str, res_str),
                SynthesisMainLayer(
                    fmaps=nf(res - 1),
                    impl=self.impl,
                    gpu=self.gpu,
            ...
            setattr(
                self,
                "torgb_{}_{}".format(res_str, res_str),
                ToRgbLayer(
                    impl=self.impl, gpu=self.gpu, name="{}x{}".format(res_str, res_str)
                ),
            )
    ...
    def call(self, dlatents_in):
        ...
        # Early layers
        x = tf.tile(
            tf.cast(self.const_4_4, "float32"), [tf.shape(dlatents_in)[0], 1, 1, 1]
        )
        ...
        # Main layers
        for res in range(3, self.resolution_log2 + 1):
            x = getattr(self, "layer_{}_{}_up".format(2 ** res, 2 ** res))(
                x, dlatents_in[:, res * 2 - 5]
            )
            x = getattr(self, "layer_{}_{}".format(2 ** res, 2 ** res))(
                x, dlatents_in[:, res * 2 - 4]
            )
            y = upsample_2d(y, k=self.resample_kernel, impl=self.impl, gpu=self.gpu)
            y = getattr(self, "torgb_{}_{}".format(2 ** res, 2 ** res))(
                x, dlatents_in[:, res * 2 - 3], y
            )
...
class StyleGan2Generator(tf.keras.layers.Layer):
    ...
    def __init__(self, resolution=1024, weights=None, impl="cuda", gpu=True, **kwargs):
        ...
        self.mapping_network = MappingNetwork(
            resolution=self.resolution, name="Mapping_network"
        )
        self.synthesis_network = SynthesisNetwork(
            resolution=self.resolution, impl=impl, gpu=gpu, name="Synthesis_network"
        )
    ...
    def __load_weights(self, weights_name):
        ...
        if (weights_name in available_weights) and type(weights_name) == str:
            data = np.load(
                weights_stylegan2_dir + weights_name + ".npy", allow_pickle=True
            )[()]

            weights_mapping = [data.get(key) for key in mapping_weights]
            weights_synthesis = [
                data.get(key) for key in synthesis_weights[weights_name]
            ]
