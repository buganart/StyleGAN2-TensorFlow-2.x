...
class StyleGan2Discriminator(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.from_rgb = FromRgbLayer(
            fmaps=nf(self.resolution_log2 - 1),
        ...
        for res in range(self.resolution_log2, 2, -1):
            ...
            setattr(
                self,
                "block_{}_{}".format(res_str, res_str),
                BlockLayer(
                    res=res,
        ...
        # last layers
        self.conv_4_4 = Conv2DLayer(
            fmaps=nf(1), kernel=3, impl=self.impl, gpu=self.gpu, name="4x4/Conv"
        )
        self.conv_4_4_bias = self.add_weight(
            name="4x4/Conv/bias",
            shape=(512,),
    ...
    def call(self, y):
        ...
        x = self.conv_4_4(x)
        x += tf.reshape(
            self.conv_4_4_bias, [-1 if i == 1 else 1 for i in range(x.shape.rank)]
        )
    ...
    def __load_weights(self, weights_name):
        ...
        if (weights_name in available_weights) and type(weights_name) == str:
            ...
            weights_discriminator = [
                data.get(key) for key in discriminator_weights[weights_name]
            ]
