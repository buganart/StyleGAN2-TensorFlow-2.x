...
class FromRgbLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.conv2d_rgb = Conv2DLayer(
            fmaps=self.fmaps, kernel=1, impl=self.impl, gpu=self.gpu, name="FromRGB"
        )

        self.rgb_bias = self.add_weight(
            name="FromRGB/bias",
            shape=(self.fmaps,),
    ...
    def call(self, x, y):
        ...
        t += tf.reshape(
            self.rgb_bias, [-1 if i == 1 else 1 for i in range(t.shape.rank)]
        )
        t = tf.math.multiply(tf.nn.leaky_relu(t, 0.2), tf.math.sqrt(2.0))
