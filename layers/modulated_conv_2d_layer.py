...
class ModulatedConv2DLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.mod_bias = self.add_weight(
            name="mod_bias",
            shape=(input_shape[1],),
            initializer=tf.random_normal_initializer(0, self.init_std_s),
            trainable=True,
        )
        self.mod_weight = self.add_weight(
            name="mod_weight",
            shape=(self.latent_size, input_shape[1]),
            initializer=tf.random_normal_initializer(0, self.init_std_s),
            trainable=True,
        ...
        self.weight = self.add_weight(
            name="weight",
            shape=(self.kernel, self.kernel, input_shape[1], self.fmaps),
            initializer=tf.random_normal_initializer(0, self.init_std_w),
            trainable=True,
        )
    ...
    def call(self, x, dlatent_vect):
        ...
        # Modulate.
        s = tf.matmul(
            dlatent_vect, tf.math.multiply(self.mod_weight, self.runtime_coef_s)
        )  # [BI] Transform incoming W to style.
        s = (
            tf.nn.bias_add(s, tf.math.multiply(self.mod_bias, self.lrmul)) + 1
        )  # [BI] Add bias (initially 1).

        ww = tf.math.multiply(
            ww, tf.cast(s[:, np.newaxis, np.newaxis, :, np.newaxis], w.dtype)
        ...
        if self.fused_modconv:
            x = tf.reshape(
                x, [-1, self.fmaps, x.shape[2], x.shape[3]]
