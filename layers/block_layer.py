...
class BlockLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.conv2d_0 = Conv2DLayer(
            fmaps=nf(self.res - 1), kernel=3, impl=self.impl, gpu=self.gpu, name="Conv0"
        )
        ...
        self.conv2d_1_down = Conv2DLayer(
            fmaps=nf(self.res - 2),
            kernel=3,
            down=True,
    ...
    def call(self, x):
        ...
        x += tf.reshape(
            self.bias_1_down, [-1 if i == 1 else 1 for i in range(x.shape.rank)]
        )
        x = tf.math.multiply(tf.nn.leaky_relu(x, 0.2), tf.math.sqrt(2.0))

        t = self.conv2d_skip(t)
        x = (x + t) * (1 / tf.math.sqrt(2.0))
