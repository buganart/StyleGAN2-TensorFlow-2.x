...
class NonSaturatingLogLossPLRegularizer(tf.keras.losses.Loss):
    ...
    def compute_pathreg(self, G, D, minibatch_size):
        ...
        with tf.GradientTape(watch_accessed_variables=False) as tape:
            ...
            # Compute |J*y|.
            pl_noise = tf.random.normal(tf.shape(fake_images_out)) / np.sqrt(
                np.prod(fake_images_out.shape[2:])
            )
        ...
        pl_mean = self.pl_mean_var + self.pl_decay * (
            tf.reduce_mean(pl_lengths) - self.pl_mean_var
        )
